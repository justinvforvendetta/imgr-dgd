...
class Factory:
    ...
    API_URL = "https://imgur-apiv3.p.mashape.com/"
    API_CORE_URL = "https://api.imgur.com/"
    ...
    def buildCoreRequest(self, endpoint, data = None):
        ...
        Prepends 3/ and appends \.json to the tuple-form, not the endpoint form.'''
        if isinstance(endpoint, str):
            url = self.API_CORE_URL + endpoint
        else:
            url = self.API_CORE_URL + '3/' + ('/'.join(endpoint)) + ".json"
    ...
    def buildRequestOAuthTokenSwap(self, grant_type, token):
        ...
        return self.buildCoreRequest('oauth2/token', data)
